...
pattern = re.compile(r'<!-- Logo Area -->.*?<!-- Menu Principal -->', re.DOTALL | re.IGNORECASE)
...
for f in html_files:
    ...
    try:
        ...
        if '<!-- Logo Area -->' in content and '<!-- Menu Principal -->' in content:
            new_content = pattern.sub(new_logo_block, content)
            ...
            print(f"Skipped {f} - Markers not found.")
